...
object DockingData extends App with ProgramWithSource {
  ...
  override def filename: String = "docking-data.txt"
  ...
  object Part1 extends Part {
    ...
    case class ProgramState(
        ...
        memory: Memory = Map.empty.withDefaultValue(0L),
        mask0: Integer = Long.MinValue,
        mask1: Integer = 0L
    ) {
          ...
          Some(
            instruction match {
              ...
              case Mem(address, value) =>
                ...
                copy(
                  instructions = newInstructions,
                  memory = memory + (address -> newValue)
                )
            }
          )
      }
    ...
    override def sumFinalMemory(instructions: Instructions): Integer = {
      ProgramState(instructions).execs.last.memory.values.sum
    }
  }
  ...
  object Part2 extends Part {
    ...
    case class ProgramState(
        instructions: Instructions,
        memory: Memory = Map.empty.withDefaultValue(0L),
        mask0: Address = Long.MinValue,
        mask1s: Seq[Address] = Seq(0L)
    ) {
      def execOne: Option[ProgramState] = instructions match {
        case Nil => None
        case instruction :: newInstructions =>
          Some(
            instruction match {
              ...
              case Mem(address, value) =>
                ...
                copy(
                  instructions = newInstructions,
                  memory = memory ++ addresses.map(_ -> value)
                )
            }
          )
      }
  ...
  def parseInstructions(input: String): Instructions =
    input.linesIterator.map(parseInstruction).toList

  lazy val input: String = fileSource.mkString
